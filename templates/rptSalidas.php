...
Class PDF extends TCPDF{
	...
	function Header(){
		...
		$this->Ln(10);
        $this->setX(5);
        $this->Cell(270, 0, '', 'T', 0, 'L');
        $this->Ln(1);
        $this->SetFont(PDF_FONT_NAME_MAIN, 'B', 10);
        $this->Cell(35, 5, 'FECHA', 0, 0, 'C');
        $this->Cell(55, 5, 'CLIENTE', 0, 0, 'L');
        $this->Cell(45, 5, 'PROVEEDOR', 0, 0, 'L');
        $this->Cell(40, 5, 'PRODUCTO', 0, 0, 'L');
        $this->Cell(30, 5, 'CANTIDAD', 0, 0, 'C');
        $this->Cell(30, 5, 'KILOGRAMOS', 0, 0, 'C');
        $this->Cell(30, 5, 'IMPORTE', 0, 1, 'C');
        $this->setX(5);
        $this->Cell(270, 0, '', 'T', 0, 'L');
	}
    ...
    foreach($registros as $prod){
        $relleno = $contador % 2;
        $miReporte->Cell(30, 5, $prod->fechaf, 0, 0, 'C', $relleno);
        $miReporte->Cell(65, 5, $prod->cliente, 0, 0, 'L', $relleno);
        $miReporte->Cell(40, 5, $prod->proveedor, 0, 0, 'L', $relleno);
        $miReporte->Cell(45, 5, $prod->descripcion, 0, 0, 'L', $relleno);
        $miReporte->Cell(30, 5, $prod->cantidad, 0, 0, 'C', $relleno);
        $miReporte->Cell(30, 5, number_format($prod->peso,3), 0, 0, 'C', $relleno);
        $miReporte->Cell(30, 5, number_format($prod->importe,3), 0, 1, 'C', $relleno);
        $this->total = $this->total + $prod->peso;
        $this->importeTotal = $this->importeTotal + $prod->importe;
        $contador = $contador + 1;
    }
    ...
    $miReporte->Cell(35, 5, number_format($this->importeTotal,3), 0, 0, 'R');
    $miReporte->Output();
exit();
?>