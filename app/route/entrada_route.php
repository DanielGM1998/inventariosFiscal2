<?php
	...
	$app->group('/entrada/', function () {
		...
		// Obtener entradas entre fechas xlsx
		$this->get('getByDate/xlsx/{inicio}/{fin}', function($request, $response, $arguments){
			...
			$sheet->setCellValue("A1", 'ENTRADAS ALMACEN GENERAL');
			$sheet->setCellValue("E1", $subtitulo);

			$sheet->setCellValue("A2", 'Fecha');
			$sheet->setCellValue("B2", 'Proveedor');
			$sheet->setCellValue("C2", "Nota");
			$sheet->setCellValue("D2", 'Peso');
			$sheet->setCellValue("E2", 'Valor');

			$entradas = $this->model->entrada->getByDate($arguments['inicio'], $arguments['fin'])->result;

			$fila = 3;
			foreach($entradas as $res){
				$sheet->setCellValue("A".$fila, $res->fecha);
				$sheet->setCellValue("B".$fila, $res->proveedor);
				$sheet->setCellValue("C".$fila, $res->nota_entrada);
				$sheet->setCellValue("D".$fila, $res->peso_total);
				$sheet->setCellValue("E".$fila, $res->valor);
				$fila++;
			}
			$writer = new Xlsx($spreadsheet);
			header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
			header("Content-Disposition: attachment; filename=\"entradas_".date('YmdHi').".xlsx\"");
		...
		$this->put('cancel/{entrada}', function($request, $response, $arguments) {
			...
			if(!isset($_SESSION['usuario'])){
				...
				for($x=0;$x<$count;$x++){
					...
					if($cant <= $checkStock->result->stock) {
						$stockrest = $this->model->producto->stockRest($cant, $prod);
						if(!$stockrest->response) {
							$stockrest->state=$this->model->transaction->regresaTransaccion();
