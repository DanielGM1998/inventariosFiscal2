<?php
	...
	class UsuarioModel {
		...
		public function getAll($pagina, $limite, $usuario_tipo_id, $busqueda) {
			...
			$usuario = $this->db
				->from($this->table)
                ->select(null)->select("id_usuario, nombre, apellidos, email, tipo, username, CONCAT_WS(' ',
				nombre, apellidos) as nomcom, IF(tipo = 1, 'Administrador', IF(tipo = 2, 'Almacen General', 'Otro')) as tipos, estado, telefono, ultimo_acceso")
		...
		public function get($id) {
			$usuario = $this->db
				->from($this->table)
				//->select(null)->select("id_usuario, nombre, apellidos, email, tipo, username")
				->where('id_usuario', $id)
				->fetch();
			if($usuario) {
		...
		public function getCajeros() {
			$cajeros = $this->db
				->from($this->table)
                ->select(null)->select("id_usuario, nombre, apellidos, email, tipo, username, CONCAT_WS(' ',
				nombre, apellidos) as nomcom")
				->where("tipo", 4)
				->where("estado", 1)
				->orderBy('apellidos ASC, nombre ASC')
				->fetchAll();
			$this->response->result = $cajeros;
			$this->response->total = $this->db
				->from($this->table)
				->select(null)->select('COUNT(*) Total')
				->where("tipo", 4)
		...
		// Obtener los cajeros activos
		public function getCajerosActivos() {
			$cajeros = $this->db
				->from($this->table)
                ->select(null)->select("id_usuario, CONCAT_WS(' ', apellidos, nombre) as nomcom")
				->where("tipo", 4)
				->where("estado", 1)
				->orderBy('apellidos ASC, nombre ASC')
				->fetchAll();
		...
		// Obtener los cajeros activos
		public function getCajeroActivo($cajero) {
			$cajeros = $this->db
				->from($this->table)
                ->select(null)->select("id_usuario, CONCAT_WS(' ', apellidos, nombre) as nomcom")
				->where("id_usuario", $cajero)
				->where("tipo", 4)
				->where("estado", 1)
				->orderBy('apellidos ASC, nombre ASC')
				->fetchAll();			
			if($cajeros==null){
				return $this->response->SetResponse(false, 'El cajero no existe');
			}else{
				$this->response->result = $cajeros;
				return $this->response->SetResponse(true);
			}
