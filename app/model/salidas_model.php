	...
	class SalidaModel {
        ...
        public function getByDate($inicio, $fin) {
			return $this->db->getPdo()->query(
				"SELECT $this->table.id_salida, $this->table.fk_cliente, $this->table.fk_cajero, $this->table.peso_total, $this->table.total, CONCAT_WS(' ',   
        ...
        public function fk_entrada($terminacion, $field, $_peso_total, $value, $_cajero, $fecha) {
			//$data['fecha'] = date('Y-m-d H:i:s');
			$data['fecha'] = $fecha;
			$data['fecha_modificacion'] = date('Y-m-d H:i:s');
