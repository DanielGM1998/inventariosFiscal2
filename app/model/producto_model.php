<?php
	...
	class ProductoModel {
		...
		public function stockByClave2($fecha) {
			...
			$registro = $this->db->getPdo()->query(
				"SELECT
					$this->tableP.clave, CONCAT($this->tableP.nombre,' ',$this->tableP.apellidos) AS proveedor, SUM(REPLACE($this->table.peso,',','')) as peso,
					IFNULL((SELECT final FROM kardex where $this->table.id_producto = $this->tableK.fk_producto and DATE_FORMAT($this->tableK.fecha, '%Y-%m-%d') <= '$fecha' order by fecha desc limit 1 ), 0) AS exist				
				FROM cat_producto 
				INNER JOIN $this->tableP ON $this->table.fk_proveedor = $this->tableP.id_proveedor
				WHERE $this->table.estado = 1
				GROUP BY proveedor
				ORDER BY $this->tableP.clave, proveedor;"
			)->fetchAll();
			return $registro;
		}
