<?php
	...
	class DetentradaTienditaModel {
		private $db;
		private $table = 'det_entrada_tiendita';
		private $tableE = 'entrada';
		private $tableEC = 'entrada_tiendita';
		...
		public function add($data) {
			...
			try{
				...
				$this->response->SetResponse(false, "catch: Add model det_entrada_tiendita $ex");
			}
			return $this->response;
		}

		// Editar det_entrada_tiendita
		public function editByEntrada($data, $id, $idProducto) {
			...
			try{
				...
				$this->response->errors = $ex;
				$this->response->SetResponse(false, "catch: Edit model det_entrada_tiendita $ex");
			}
			return $this->response;
		}

		// Obtener det_entrada_tiendita por id
        ...
        public function getById($id) {
			return $this->response->result = $this->db->getPdo()->query(
				...
				(SELECT DATE_FORMAT(fecha,'%Y-%m-%d') FROM entrada_tiendita WHERE id_entrada = fk_entrada) AS fecha, 
				(SELECT fk_cajero FROM entrada_tiendita WHERE id_entrada = fk_entrada) AS cajero 
			FROM det_entrada_tiendita WHERE id_det_entrada = $id;"
			)->fetch();
		}

		// Eliminar det_entrada_tiendita
		...
		public function getPesoTotal($entrada) {
			return $this->db->getPdo()->query(
				"SELECT round(SUM(peso),3) as peso_total FROM det_entrada_tiendita WHERE fk_entrada = $entrada and estado=1"
			)->fetch();
		}
        ...
        public function getRptProv($inicio, $fin, $prov) {
			return $this->db->getPdo()->query(
				"SELECT $this->table.id_det_entrada, $this->table.fk_producto, $this->table.cantidad, $this->table.peso, 
				$this->tableProd.descripcion, $this->tableE.nota_entrada, $this->tableE.fk_proveedor, DATE_FORMAT($this->tableE.fecha, '%d-%m-%Y %H:%i:%s') AS fechaf, 
					...
					(SELECT SUM(replace($this->tableE.peso_total,',','')) FROM $this->tableE WHERE $this->tableE.fk_proveedor = $prov 
							AND DATE_FORMAT($this->tableE.fecha, '%Y-%m-%d') BETWEEN DATE_FORMAT('$inicio', '%Y-%m-%d') AND DATE_FORMAT('$fin', '%Y-%m-%d') AND $this->tableE.estado = 1) as peso_total , fecha
        ...
        public function getRptCve($inicio, $fin, $cve) {
			...
			return $this->db->getPdo()->query(
				"SELECT $this->table.id_det_entrada, $this->table.fk_producto, $this->table.cantidad, $this->table.peso, 
				$this->tableProd.descripcion, $this->tableProd.fk_proveedor, DATE_FORMAT($this->tableEC.fecha, '%d-%m-%Y %H:%i:%s') AS fechaf, 
				CONCAT($this->tableProv.nombre, ' ', $this->tableProv.apellidos) as proveedor, 
				(SELECT SUM(replace($this->tableEC.peso_total,',','')) FROM $this->tableEC WHERE DATE_FORMAT($this->tableEC.fecha, '%Y-%m-%d') BETWEEN DATE_FORMAT('$inicio', '%Y-%m-%d') AND DATE_FORMAT('$fin', '%Y-%m-%d')
				AND $this->tableEC.estado = 1) as peso_total 
		...
		public function getAll($pagina, $limite, $busqueda) {
			...
			$this->response->result = $this->db
				->from($this->table)
				->select(null)->select("$this->table.id_det_entrada, $this->table.fk_producto, $this->table.cantidad, $this->table.peso")
                ->where("CONCAT_WS(' ', $this->table.id_det_entrada, $this->table.fk_producto, $this->table.cantidad, $this->table.peso) LIKE '%$busqueda%'")
				->where("$this->table.estado", 1)
