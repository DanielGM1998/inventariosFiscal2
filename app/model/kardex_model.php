<?php
	...
	class KardexModel {
        ...
        // Obtener inicial de kardex por fecha
        ...
        // Obtener final de kardex por fecha
		...
		// Obtener inicial de kardex por fecha y tipo de kardex
		public function kardexInicialByKardex($terminacion, $fecha, $fk_producto, $condicion, $_cajero) {
		...
		// Obtener final de kardex por fecha y tipo de kardex
		public function kardexFinalByKardex($terminacion, $fecha, $fk_producto, $condicion, $_cajero) {
			$kardexBy = $terminacion=='entrada_tiendita'
				?	'kardex_tiendita'
				:	($terminacion=='entrada_produccion'
					?	'kardex_produccion'
		...
		public function add($data) {
			...
			try{
				...
				$this->response->SetResponse(false, "catch: Add model kardex $ex");
			}
			return $this->response;
		}

		// Agregar kardex por tipo de kardex
		public function addByKardex($data, $value, $terminacion) {
			...
			try{
				...
				$this->response->errors = $ex;
				return $this->response->SetResponse(false, "catch: Add model kardex $ex");
			}
			// return $this->response;
		}
		...
		public function entradasSum($cantidad, $fk_producto, $fecha) {
			...
			$entradasSum = $this->editByProducto($data, $fk_producto, $fecha)->result;
			$this->response->result = $entradasSum;
			return $this->response->SetResponse(true);
		}

        // Suma inicial y final en kardex
		...
		public function arreglaKardex($producto, $fecha) {
			...
			for($x=0;$x<$count;$x++){	
				...
				$entradas = ($arreglaKardex[$x]->entradas);
				$salidas = ($arreglaKardex[$x]->salidas);				
				if($inicial == -1){
					$inicial = ($arreglaKardex[$x]->inicial);
					$final = ($arreglaKardex[$x]->final);
				}else{
					$inicial = $final;
		...
		// Editar kardex por tipo de kardex
		...
		public function salidasSum($cantidad, $fk_producto, $fecha) {
			...
			return $SalidasRest = $this->editByProducto($data, $fk_producto, $fecha);
			/* $this->response->result = $SalidasRest;
			return $this->response->SetResponse(true); */
		}
		...
		// Editar kardex por producto
		public function editByProducto($data, $id, $fecha) {
			$data['fecha_modificacion'] = date('Y-m-d H:i:s');
			try{
				$this->response->result = $this->db
					->update($this->table, $data)
		...
		// Editar kardex por producto, fecha y cajero
		...
		// Editar kardex por producto, fecha mayor y cajero
		...
		// Editar kardex por producto y fecha
        ...
        public function getKardex($producto, $inicio, $fin) {
			...
			return $this->db->getPdo()->query(
				...
				,(SELECT peso FROM $this->tableProd WHERE id_producto = fk_producto) AS peso
				,(SELECT SUM(entradas) FROM $this->table WHERE fk_producto = $producto 
				AND DATE_FORMAT(fecha, '%Y-%m-%d') BETWEEN DATE_FORMAT('$inicio', '%Y-%m-%d') AND DATE_FORMAT('$fin', '%Y-%m-%d') ORDER BY fecha) as total_entradas
				,(SELECT SUM(salidas) FROM kardex WHERE fk_producto = $producto 
				AND DATE_FORMAT(fecha, '%Y-%m-%d') BETWEEN DATE_FORMAT('$inicio', '%Y-%m-%d') AND DATE_FORMAT('$fin', '%Y-%m-%d') ORDER BY fecha) as total_salidas
				FROM $this->table 
				WHERE fk_producto = $producto
